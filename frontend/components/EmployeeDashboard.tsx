...
export default function EmployeeDashboard({
  ...
  useEffect(() => {
    if (isEligibleForTest && assignedTests.length > 0) {
      const justSubmitted = sessionStorage.getItem("just_submitted_application");
      if (justSubmitted === "true") {
        const pendingTest = assignedTests.find(
          t => t.status === TestStatus.NOT_STARTED
        );
        if (pendingTest) {
  ...
  const handleClosePopup = () => {
    sessionStorage.removeItem("just_submitted_application");
    if (popupTest) {
