...
const PREDEFINED_TECH_SKILLS = [
  "React", "React Native", "TypeScript", "JavaScript", "Node.js", "Express.js", 
  "Python", "FastAPI", "Django", "Java", "Spring Boot", "C++", "C#", ".NET", 
  "PHP", "Laravel", "Ruby", "Ruby on Rails", "Go (Golang)", "Rust", "SQL", 
  "PostgreSQL", "MySQL", "MongoDB", "Redis", "Docker", "Kubernetes", "AWS", 
  "Google Cloud (GCP)", "Azure", "GraphQL", "REST API", "Tailwind CSS", 
  "HTML5/CSS3", "Git/GitHub", "CI/CD", "Unit Testing", "System Architecture", 
  "Machine Learning", "Data Engineering", "Vue.js", "Angular", "Next.js", "Svelte"
];

const PREDEFINED_EXPERIENCE_SKILLS = [
  "Full Stack Developer", "Software Engineer", "Frontend Developer", "Backend Developer", 
  "DevOps Engineer", "Data Scientist", "Product Manager", "QA / Test Engineer", 
  "UI/UX Designer", "1 Year Experience", "2 Years Experience", "3+ Years Experience", 
  "5+ Years Experience", "Team Leadership", "Agile / Scrum", "Project Management",
  "Internship Experience", "Open Source Contributor"
];
...
export default function ApplicationForm({
  ...
  // Search Engine & Auto-Suggest Dropdown States
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (otherContainerRef.current && !otherContainerRef.current.contains(event.target as Node)) {
  ...
  const filteredTechSuggestions = PREDEFINED_TECH_SKILLS.filter(s => 
    !technicalSkills.includes(s) && 
    (techSkillInput.trim() === "" || s.toLowerCase().includes(techSkillInput.trim().toLowerCase()))
  );

  const filteredOtherSuggestions = PREDEFINED_EXPERIENCE_SKILLS.filter(s => 
    !otherSkills.includes(s) && 
    (otherSkillInput.trim() === "" || s.toLowerCase().includes(otherSkillInput.trim().toLowerCase()))
  );
  ...
  async function handleSubmitForm(type: "draft" | "submitted") {
    ...
    try {
      ...
      if (res.ok) {
        const isUpdate = formStatus === ApplicationStatus.SUBMITTED;
        ...
        if (type === "submitted") {
          sessionStorage.setItem("just_submitted_application", "true");
        }
        setMessage(
          type === "draft" 
            ? "Your application draft has been saved successfully." 
            : isUpdate
              ? "Your submitted application details have been updated successfully!"
              : "Onboarding Application form submitted! You are now eligible to complete the assigned assessments."
        );
        await onRefreshAll();
        if (type === "submitted" && !isUpdate) {
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      ...
      <div className="border-b border-slate-200 pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
          {formStatus === ApplicationStatus.SUBMITTED && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 shadow-xs">
              <CheckCircle className="h-4 w-4 text-indigo-600" /> Application Submitted
            </span>
          )}
          {formStatus === ApplicationStatus.APPROVED && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-xs">
              <CheckCircle className="h-4 w-4 text-emerald-600" /> Status: Verified & Approved
            </span>
          )}
          {formStatus === ApplicationStatus.REJECTED && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200 shadow-xs">
              <AlertTriangle className="h-4 w-4 text-rose-600" /> Action Required (Rejected)
            </span>
          )}
          {formStatus === ApplicationStatus.DRAFT && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 shadow-xs">
              <Save className="h-4 w-4 text-amber-600" /> Draft Saved
            </span>
          )}
          {formStatus === ApplicationStatus.NOT_STARTED && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200 shadow-xs">
              <Clock className="h-4 w-4 text-slate-400" /> Not Submitted
            </span>
          )}
        </div>
      </div>
      ...
      {displayStatusBanner && (
        <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl text-indigo-900 text-xs flex items-start gap-3 shadow-xs">
          <CheckCircle className="h-5 w-5 text-indigo-600 mt-0.5 flex-shrink-0" />
          <div className="space-y-1">
            <h4 className="font-bold text-indigo-950">
              {formStatus === ApplicationStatus.APPROVED ? "Application Verified & Approved" : "Application Submitted & Locked"}
            </h4>
            <p className="text-indigo-800 leading-normal">
              {formStatus === ApplicationStatus.APPROVED 
                ? "Your onboarding information has been verified and approved by administration. Your records are locked for compliance."
                : "Your onboarding application has been submitted and locked for administrative review. Applications can only be submitted once."}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-6 text-xs">
        ...
        <div className="space-y-4 pt-4 border-t border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-slate-500 font-bold block mb-1">Primary Technical Skills</label>
               ...
               {!isAlreadySubmitted && (
                <div className="relative" ref={techContainerRef}>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setTechSkillInput(e.target.value);
                        ...
                        placeholder="Search or type a skill (e.g. React, Python, Java)..."
                        className="w-full bg-white border border-slate-200 text-slate-900 rounded-xl p-2.5 pl-9 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    <button
                      type="button"
                  ...
                  {showTechDropdown && filteredTechSuggestions.length > 0 && (
                    <div className="absolute z-20 top-full left-0 right-0 mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto divide-y divide-slate-100 text-xs">
                      {filteredTechSuggestions.map((skill, idx) => (
                        <div
                          key={idx}
                          onClick={() => {
                          ...
                          className="p-2.5 px-3.5 hover:bg-indigo-50 hover:text-indigo-700 text-slate-700 font-semibold cursor-pointer flex items-center justify-between transition-colors"
                        >
                          <span>{skill}</span>
                          <Plus className="h-3.5 w-3.5 text-indigo-500 opacity-60" />
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex flex-wrap gap-1.5 p-3.5 bg-slate-50 border border-slate-200 rounded-xl min-h-[70px]">
                {technicalSkills.length === 0 ? (
                  <span className="text-[10px] text-slate-400 font-mono italic">No technical skills added</span>
                ) : (
                  technicalSkills.map((tag, i) => (
                    <span key={i} className="bg-indigo-50 text-indigo-700 border border-indigo-200 px-2.5 py-1 rounded-lg font-bold flex items-center gap-1.5 text-[10px]">
                      {tag}
                      {!isAlreadySubmitted && (
                        <button type="button" onClick={() => handleRemoveTechSkill(tag)} className="hover:text-indigo-900 text-indigo-400 font-black">
                          <X className="h-3 w-3" />
                        </button>
                      )}
                    </span>
            ...
            {/* Other / Soft Skills */}
            <div className="space-y-2">
              <label className="text-slate-500 font-bold block mb-1">Past Experience</label>
               ...
               {!isAlreadySubmitted && (
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. 2 Years at Google (Press Enter or Add Experience)"
                  ...
                  <button
                    ...
                    className="bg-slate-900 hover:bg-slate-800 text-white font-bold px-4 rounded-xl transition cursor-pointer text-xs shrink-0"
                  >
                    Add Experience
                  </button>
                </div>
              ...
              <div className="flex flex-wrap gap-1.5 p-3.5 bg-slate-50 border border-slate-200 rounded-xl min-h-[70px]">
                {otherSkills.length === 0 ? (
                  <span className="text-[10px] text-slate-400 font-mono italic">No past experience added</span>
                ) : (
